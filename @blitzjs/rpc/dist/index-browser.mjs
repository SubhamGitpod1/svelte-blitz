...
function useQuery(queryFn, params, options = {}) {
  ...
  const { data, ...queryRest } = useQuery$1({
    ...
    queryFn: routerIsReady ? () => enhancedResolverRpcClient(params, { fromQueryHook: true }) : emptyQueryFn,
    ...options,
    enabled
  });
  if (queryRest.isIdle && isServer && suspenseEnabled !== false && !data && (!options || !("suspense" in options) || options.suspense) && (!options || !("enabled" in options) || options.enabled)) {
    const e = new Error();
...
function useMutation(mutationResolver, config) {
  ...
  const { mutate, mutateAsync, ...rest } = useMutation$1((variables) => enhancedResolverRpcClient(variables, { fromQueryHook: true }), {
    throwOnError: true,
    ...config
  });
...
function invokeWithCtx(queryFn, params, ctx) {
  if (typeof queryFn === "undefined") {
    throw new Error("invokeWithCtx is missing the first argument - it must be a query or mutation function");
  }
...
export { BlitzRpcPlugin, __internal_buildRpcClient, getInfiniteQueryKey, getQueryClient, getQueryKey, invalidateQuery, invoke, invokeWithCtx, normalizeApiRoute, setQueryData, useInfiniteQuery, useMutation, usePaginatedQuery, useQuery };
