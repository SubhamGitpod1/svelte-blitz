...
async function transformBlitzRpcServer(src, id, root, resolvers, options) {
	...
	for (let resolverFilePath of resolvers) {
		const relativeResolverPath = path.posix.relative(
			path.dirname(id),
			path.join(root, resolverFilePath)
		);
...
async function recursiveFindResolvers(dir, filter, ignore, arr = [], rootDir = dir) {
	...
	if (dir === rootDir) {
		folders = folders.filter((folder) =>
			loaderUtils.topLevelFoldersThatMayContainResolvers.includes(folder)
		);
	}
	await Promise.all(
		folders.map(async (part) => {
